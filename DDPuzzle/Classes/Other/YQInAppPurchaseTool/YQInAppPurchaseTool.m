...
- (void)paymentQueue:(SKPaymentQueue *)queue updatedTransactions:(NSArray *)transactions
{
    ...
    for (SKPaymentTransaction *transaction in transactions) {
        NSLog(@"队列状态变化 %ld", transaction.transactionState);
        // 如果小票状态是购买完成
...
//            [self.delegate IAPToolRestoredProductID:transaction.payment.productIdentifier error:nil];
            [[SKPaymentQueue defaultQueue] finishTransaction:transaction];
...
- (void)paymentQueueRestoreCompletedTransactionsFinished:(SKPaymentQueue *)queue {
    if (queue.transactions.count > 0) {
        [self.delegate IAPToolRestoredWithResult:YES];
    } else {
        [self.delegate IAPToolRestoredWithResult:NO];
    }
}
