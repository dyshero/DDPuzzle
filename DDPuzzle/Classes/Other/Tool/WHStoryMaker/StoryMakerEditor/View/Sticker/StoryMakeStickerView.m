...
{
    if (self = [super init]) {
        _stickerArray = [NSArray arrayWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"sticker" ofType:@"plist"]];
        [self configureView];
...
- (NSArray<UIViewController *> *)viewControllerArray
{
    if (!_viewControllerArray) {
        ...
        NSInteger i;
        for (i = 0; i < _stickerArray.count; ++i) {
            NSDictionary *dic = _stickerArray[i];
            StoryMakeStickerFootSingleViewController *singleVC = [[StoryMakeStickerFootSingleViewController alloc] initWithType:(StoryMakeStickerFootSingleType)i imageUrlArray:dic[@"arry"]];
